...
public class AppNavigation {
	...
	// Creo que esta clase se puede hacer una interfaz y que otras clases de
	// actividad tengan estas funciones...
	...
	public static void openUserConfig(Activity CurrentActivity) {
		...
		// myIntent.putExtra("key", value); //Optional parameters
		CurrentActivity.startActivityForResult(myIntent,
	...
	public static void logOut(final Activity CurrentActivity) {
		if (!logginOut) {
			logginOut = true;
	...
	public static String getUserId() {
		String userId = AppNavigation.getCookie(AppNavigation.siteDomain,
				"ebentum_userid");
		return userId;
	}
	...
	public static boolean processWebNavigation(Activity CurrentActivity,
			WebView view, String url, String fragmentName) {
		...
		if (url.startsWith(siteUrl)) {
			...
			if (CurrentActivity != null) {
				activityName = CurrentActivity.getClass().getName();
			}
			...
			// si estamos deslogeando no hacer nada. es porque saltaba la
			// navegacion en las
			// tres pesta–as al cerrar sesion porque ha abierto de nuevo
			// mainactivity
			if (AppNavigation.logginOut && ( (activityName != null && !activityName.endsWith("LoginActivity")) || activityName == null )  )
				return true;
			...
			if (pageUrl.startsWith("users/")) {
				...
				if (userID.startsWith(getUserId())) {
					...
					if (activityName.endsWith("UserSettingsActivity")) {
						// Volver hacia atras desde configuracion de usuario a
						// pantalla
					...
					// el resto de casos abrir en una pantalla nueva el perfil
					// de usuario
			...
			if (pageUrl.startsWith("events/search")) {
				...
				// Necesario para que la pagina de busqueda cargue correctamente
				// la –apa
				if (pageUrl.contains("?"))
			...
			if (pageUrl.startsWith("events/") && pageUrl.endsWith("/edit")) {
				...
				String eventID = pageUrl.replace("events/", "").replace(
						"/edit", "");
			...
			if (pageUrl.startsWith("welcome")) {
				// view.loadUrl(url + "?native_app=true");
				if (activityName != null && activityName.endsWith("LoginActivity")) {
					// Solo la pantalla de login puede cargar esta ruta. Si no
					// se entienede que
			...
			if (pageUrl != null && pageUrl.length() > 0 && !pageUrl.startsWith("#_=_") && !pageUrl.startsWith("?")){
				...
				if (activityName != null && activityName.endsWith("LoginActivity")) {
					...
					if (AppNavigation.logginOut) {
						// intento de cargar la ruta raiz para loguear desde el
						// login no hacer nada
						return false;
					} else {
					*/
